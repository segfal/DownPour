...
void Application::recordCommandBuffer(VkCommandBuffer cmd, uint32_t imageIndex, uint32_t frameIndex) {
    ...
    if (roadModel.getIndexCount() > 0) {
        ...
        if (roadModel.hasTexture()) {
            // Road has textures - use car pipeline approach
            ...
            if (!roadMaterials.empty()) {
                ...
                for (size_t i = 0; i < roadMaterials.size(); i++) {
                    ...
                    // Bind base color texture (descriptor set 1)
                    ...
                    // For simplicity, reuse car descriptor pool
                    // In production, you'd want separate descriptors for road
                    // For now, just use the world pipeline for textured road
                    vkCmdDrawIndexed(cmd, material.indexCount, 1, material.indexStart, 0, 0);
                }
            }
        } else {
            // Road has no textures - use simple world pipeline
...
VkFormat Application::findSupportedFormat(const std::vector<VkFormat>& candidates,
  ...
  void Application::loadCarModel() {
      ...
      // Note: Model matrix will be set by updateCarPhysics() each frame
      // No need to set it here - physics update handles position, rotation, and scale
      ...
      std::cout << "=== MODEL BOUNDING BOX (UNSCALED) ===" << std::endl;
      std::cout << "Min: (" << minB.x << ", " << minB.y << ", " << minB.z << ")" << std::endl;
      std::cout << "Max: (" << maxB.x << ", " << maxB.y << ", " << maxB.z << ")" << std::endl;
      std::cout << "Scaled Min: (" << minB.x * carScaleFactor << ", " << minB.y * carScaleFactor << ", " << minB.z * carScaleFactor << ")" << std::endl;
      std::cout << "Scaled Max: (" << maxB.x * carScaleFactor << ", " << maxB.y * carScaleFactor << ", " << maxB.z * carScaleFactor << ")" << std::endl;
      std::cout << "Center: (" << carModel.getCenter().x * carScaleFactor << ", " << carModel.getCenter().y * carScaleFactor << ", " << carModel.getCenter().z * carScaleFactor << ")" << std::endl;
      std::cout << "======================================\n" << std::endl;
      ...
      std::cout << "=== SUGGESTED COCKPIT OFFSET ===" << std::endl;
      std::cout << "Based on bounding box analysis:" << std::endl;
      std::cout << "  X: " << suggestedX << " (centered)" << std::endl;
      std::cout << "  Y: " << suggestedY << " (forward position)" << std::endl;
      std::cout << "  Z: " << suggestedZ << " (eye level)" << std::endl;
      std::cout << "Current offset: (" << cockpitOffset.x << ", " << cockpitOffset.y << ", " << cockpitOffset.z << ")" << std::endl;
      std::cout << "=================================\n" << std::endl;

      // Debug: Print car model info
      std::cout << "=== CAR MODEL INFO ===" << std::endl;
      std::cout << "Index count: " << carModel.getIndexCount() << std::endl;
      std::cout << "Has texture: " << (carModel.hasTexture() ? "YES" : "NO") << std::endl;
      std::cout << "Material count: " << carModel.getMaterials().size() << std::endl;
      std::cout << "======================\n" << std::endl;
  }

  void Application::loadRoadModel() {
      roadModel.loadFromFile("assets/models/road.glb", device, physicalDevice,
                            commandPool, graphicsQueue);

      std::cout << "\n=== ROAD MODEL INFO ===" << std::endl;
      std::cout << "Index count: " << roadModel.getIndexCount() << std::endl;
      glm::vec3 dimensions = roadModel.getDimensions();
      std::cout << "Dimensions: " << dimensions.x << " x "
                << dimensions.y << " x " << dimensions.z << std::endl;
      std::cout << "Has texture: " << (roadModel.hasTexture() ? "YES" : "NO") << std::endl;
      std::cout << "Material count: " << roadModel.getMaterials().size() << std::endl;
  ...
  void Application::updateCameraForCockpit() {
      ...
      glm::vec3 cockpitPosition = carPosition + rotatedOffset + glm::vec3(0.0f, 0.4f, -0.15f); // x is forward and backward, y is up and down, z is left and right
      ...
      if (!printedOnce) {
          ...
          std::cout << "Car world position: (" << carPosition.x << ", " << carPosition.y << ", " << carPosition.z << ")" << std::endl;
          std::cout << "Car rotation (Y-axis): " << carRotation << " degrees" << std::endl;
          std::cout << "Cockpit offset (before rotation): (" << cockpitOffset.x << ", " << cockpitOffset.y << ", " << cockpitOffset.z << ")" << std::endl;
          std::cout << "Rotated offset: (" << rotatedOffset.x << ", " << rotatedOffset.y << ", " << rotatedOffset.z << ")" << std::endl;
          std::cout << "Final camera position: (" << cockpitPosition.x << ", " << cockpitPosition.y << ", " << cockpitPosition.z << ")" << std::endl;
          std::cout << "Camera is " << (cockpitPosition.y >= 0 ? "ABOVE" : "BELOW") << " ground (Y=" << cockpitPosition.y << ")" << std::endl;
          std::cout << "===========================\n" << std::endl;
          printedOnce = true;
      }
...
void Application::updateDebugMarkers() {
    ...
    glm::vec3 corners[8] = {
        transformPoint(glm::vec3(bbMin.x, bbMin.y, bbMin.z)),
        transformPoint(glm::vec3(bbMax.x, bbMin.y, bbMin.z)),
        transformPoint(glm::vec3(bbMax.x, bbMax.y, bbMin.z)),
        transformPoint(glm::vec3(bbMin.x, bbMax.y, bbMin.z)),
        transformPoint(glm::vec3(bbMin.x, bbMin.y, bbMax.z)),
        transformPoint(glm::vec3(bbMax.x, bbMin.y, bbMax.z)),
        transformPoint(glm::vec3(bbMax.x, bbMax.y, bbMax.z)),
        transformPoint(glm::vec3(bbMin.x, bbMax.y, bbMax.z))
    };
