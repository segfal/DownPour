...
void Application::initVulkan() {
    ...
    camera = Camera({0.0f, 0.0f, 2.0f}, aspect);
...
void Application::recordCommandBuffer(VkCommandBuffer cmd, uint32_t imageIndex, uint32_t frameIndex) {
    ...
    if (carModel.getIndexCount() > 0 && carModel.hasTexture()) {
        ...
        // Bind descriptor sets: set 0 = camera, set 1 = texture
        std::vector<VkDescriptorSet> sets = {descriptorSets[frameIndex], carDescriptorSets[frameIndex]};
        vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS,
                               carPipelineLayout, 0,
        ...
        // Draw
        vkCmdDrawIndexed(cmd, carModel.getIndexCount(), 1, 0, 0, 0);
    }
...
VkFormat Application::findSupportedFormat(const std::vector<VkFormat>& candidates,
  ...
  void Application::createDepthResources() {
    ...
    void Application::createRoadBuffers() {
      // Define road quad (100x100 units, at y=0)
  ...
  void Application::createCarDescriptorSets() {
      ...
      // Create descriptor pool for car textures
      VkDescriptorPoolSize poolSize{};
      ...
      carDescriptorSets.resize(MAX_FRAMES_IN_FLIGHT);
      if (vkAllocateDescriptorSets(device, &allocInfo, carDescriptorSets.data()) != VK_SUCCESS) {
      ...
      for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
          ...
          imageInfo.imageView = carModel.getTextureImageView();
          imageInfo.sampler = carModel.getTextureSampler();

          VkWriteDescriptorSet descriptorWrite{};
          descriptorWrite.sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
          descriptorWrite.dstSet = carDescriptorSets[i];
          descriptorWrite.dstBinding = 0;
          descriptorWrite.dstArrayElement = 0;
  ...
  void Application::updateCarPhysics(float deltaTime) {
      ...
      // Scale the car to realistic size (BMW M3 is about 4.8m long)
      modelMatrix = glm::scale(modelMatrix, glm::vec3(1.0f, 1.0f, 1.0f));
      carModel.setModelMatrix(modelMatrix);
  }

  void Application::updateCameraForCockpit() {
      // Position camera inside the car at driver's eye level
      // Typical driver eye position: slightly left of center, ~1.0m above ground,
      // about 0.3m forward from center of the car
      glm::vec3 cockpitOffset(0.3f, 0.6f, -0.3f);  // x: left/right, y: height, z: forward/back
