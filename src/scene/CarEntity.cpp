...
NodeHandle CarEntity::getSteeringWheelNode() const {
    return getNode(ROLE_STEERING_WHEEL);
}
...
void CarEntity::setSteeringAngle(float degrees) {
    ...
    NodeHandle steeringWheel = getSteeringWheelNode();
    if (!steeringWheel.isValid() || !getScene()) {
        return;
    }
    ...
    animateRotation(ROLE_STEERING_WHEEL, rotation);
}
